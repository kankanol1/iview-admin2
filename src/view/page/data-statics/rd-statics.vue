<template>
  <div>
      ...
      <Row>
        ...
        <Col :md="4" :lg="4">
          <ButtonGroup size="small" style="padding-left:10px;">
            ...
            <Button ghost @click="dataCount()" type="primary" icon="ios-speedometer-outline" :loading="loading" title="开始统计">
            </Button>
          </ButtonGroup>
        </Col>
      </Row>
    ...
    <Table
      ref="ref_1568621944471"
      :columns="columns"
      :data="content" border
      :loading="loading" :height="tableHeight"
  ...
  export default {
    ...
    methods: {
      ...
      dataCount() {
        ...
        let pointCodes = this.$refs.ref_1568621944471.getSelection().map(n => n.pointCode);
        ...
        this.$http.post('/api/com/redianshuju/countRecords', args).then((response) => {
          if (!!response && response.status === 200) {
            if ((response.data.status & 0b00001111) === 0b00000001) {
              let data = {};
