  ...
  export default {
      ...
      return {
        ...
        columns: [
          ...
          },
          {
            ...
            align: 'center',
            children: [
              {title: '车牌号码', key: 'targetPlateNumber', width: 90, align: 'center',},
              {
                title: '车牌类型', key: 'targetPlateType', align: 'center',
                render: (h, params) => h('div', {}, this.dictionary.cplx && this.dictionary.cplx[params.row.targetPlateType] || params.row.targetPlateType),
              },
              {
                title: '卡口名称', key: 'sourcePointCode', align: 'center',
                render: (h, params) => this.$createTooltip(h, params, 'left', {
                  点位编号: params.row.targetDevice && params.row.targetDevice.pointCode || params.row.sourcePointCode || '无',
                ...
                }, params.row.targetDevice && params.row.targetDevice.roadName || params.row.sourcePointCode || '无'),
              },
              {
                title: '行驶方向', key: 'targetDrivingDirection', width: 100, align: 'center',
                render: (h, params) => h('div', {}, this.dictionary.xsfx && this.dictionary.xsfx[params.row.targetDrivingDirection] || params.row.targetDrivingDirection || '无'),
              },
              {
                title: '过车时间', key: 'targetAcquisitionTime', align: 'center',
                render: (h, params) => this.$createTooltip(h, params, 'left', {
    ...
    methods: {
      ...
      selectNodes() {
        ...
        this.$http.post('/api/com/data/4/associated/data/4/kakou/selectDetailNodes', args).then((response) => {
