  ...
  <div>
    ...
    <Row>
      ...
      <Col span="8" style="position: relative">
        ...
        <Button style="position: absolute;right:0;top: 120px" size="small" @click="rightSelectDataByPointCodes()"
                type="info" icon="ios-search"/>
        <Table
          ref="selectContentColumns"
          highlight-row :columns="addDataColumns" :data="aggData" border
      ...
      <Col span="8">
        <Table
          size="small"
          ref="selection" highlight-row
          :columns="rightSelectColumns"
          :data="rightSelectData"
          border
        ...
        <Page
          size="small"
          show-total :page-size-opts="[100,200,500,1000,2000]"
    ...
    <!--
        <div ref="ref_1591249436712" style="width:100%; height: calc(100% - 10pt - var(&#45;&#45;searchBar));">
...
<script>
  export default {
    ...
    mounted() {
      ...
      const rs = (this.selectData || []).filter(it => it.device).map(it => it.device).filter(it => it.relationRedianIds)
        .map(it => it.relationRedianIds).reduce((a, b) => a.concat(b), []);
      ...
      this.$http.post('/api/com/redian/selectNodesByPointCode', {
        ...
        if (response && response.status === 200 && (response.data.status & 0b00001111) === 0b00000001) {
          ...
          let index = 0;
          (this.selectData || []).filter(it => it.device && it.device.relationRedianIds).forEach(it => { // 筛选有关联关系的内容
            let relationRedianIds = it.device.relationRedianIds; // 获取关联关系
            let groupDesc = (relationRedianIds || []).filter(it => this.selectCachePoint[it]).map(it => this.selectCachePoint[it]).map(it => it.roadName).join(',');
            (relationRedianIds || []).filter(__ => this.selectCachePoint[__]).forEach(__ => { // 判断关联关系是有效的
    ...
    data() {
      return {
        ...
        /*路由过来的数据展示方式*/ selectColumns: [
          {type: 'selection', title: 'key', width: 30, align: 'center',},
          {title: '组', key: '_index', align: 'center', width: 30},
          {
          ...
          {
            title: '车牌号码', key: 'cphm', minWidth: 70, maxWidth: 70, align: 'center',
            render: (h, params) => {
              let slot = [h('p', {}, `号牌颜色: ${params.row.hpys || '未知'}`)];
              let content = params.row.cphm || '未知';
              return h('Tooltip', {
                nativeOn: {
                  click: () => {
        ...
        /*选择的关联点位展示方式*/ selectPointColumns: [
          {type: 'selection', title: 'key', width: 30, align: 'center',},
          {title: '序', key: 'index', align: 'center', width: 30,},
          {title: '组', key: 'group', align: 'center', width: 30,},
          {
            title: '点位名称', key: 'kkbh', align: 'center', minWidth: 100,
