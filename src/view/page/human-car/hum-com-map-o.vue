<template>
  <div>
    <div class="clear">
      <Form ref="ref_formItem_r2_agg" class="form-item-s" :model="formItem_r2_agg" :rules="formItem_r2_agg_rules"
            inline>
        <FormItem prop="cplx">
          <Select class="interval" size="small" v-model="formItem_r2_agg.cplx" placeholder="请选择车牌类型"
                  @on-change="queryChange">
            <Option v-for="(value, key) in cplx" :key="key" :value="key" :label="value">{{value}}</Option>
          </Select>
        </FormItem>
        <FormItem prop="cphms">
          <Input class="interval" size="small" clearable v-model="formItem_r2_agg.cphms" placeholder="请输入车牌号码,多号码半角逗号间隔"
                 @on-change="queryChange"/>
        </FormItem>
        <FormItem prop="imsis">
          <Input class="interval" size="small" clearable v-model="formItem_r2_agg.imsis" placeholder="请输入IMSI,多号码半角逗号间隔"
                 @on-change="queryChange"/>
        </FormItem>
        <FormItem prop="datetimerange">
          <DatePicker class="interval" size="small" type="datetimerange" v-model="formItem_r2_agg.datetimerange"
    ...
    <div style="height:calc(100vh - 140px)" :style="{ overflowY: 'auto',position: 'relative'}">
      <div
        ...
        <Table
          ref="selection"
          highlight-row :columns="columns_r2_agg"
          :data="content_r2_agg" border
          :loading="loading_r2_agg"
          :max-height="tableHeight / 2 - 13"
          @on-row-click="oneClick"
          @on-row-dblclick="twoClick"
        />
        <Page
          size="small"
          show-total
    ...
    mounted() {
      ...
      if (0 < dictionaries.filter(node => !sessionStorage.getItem(node)).length) {
        this.$http.post('/api/com/dictionaries/selectNodes', {
          ...
          lxmcs: dictionaries.filter((_) => !sessionStorage.getItem(_)).join(',')
        }).then((response) => {
        ...
        columns_r2_agg: [
          {
            ...
            width: 75,
            align: 'center',
            render: (h, params) => h('div', {}, params.row.cphm),
          },
          {
            title: 'IMSI',
            key: 'imsi',
            width: 115,
            align: 'center',
            render: (h, params) => h('div', {}, params.row.imsi),
          },
          {
